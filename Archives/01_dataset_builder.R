...
# Concatenate all files from Gorilla
...
demographic<-info%>%
  filter(Question.Key == "response-3"| Question.Key ==  "gender" | Question.Key == "response-1"| Question.Key == "DoB-year"| Question.Key == "respondent-email")%>%
  select( "Participant.Public.ID" ,"Participant.Private.ID","Question.Key","Response")%>%
  spread(Question.Key,Response)%>%
  select( "Participant.Public.ID" ,"DoB-year","gender","response-1")%>%
  'colnames<-'(c("Pt.code" ,"Pt.age","Pt.gender","Pt.study"))

  
# Combine the Gorilla and Demographic datasets using Pt.code as the key
...
dataset <- temp%>%
  mutate(Wheel.y = Wheel.y - 300,
    ...
    Pt.gender = ifelse(Pt.gender == "Donna", "female", "male"),
    Pt.gender = factor(Pt.gender, levels = c("female","male")),
    Pt.study = parse_number(Pt.study),
    ...
    Resp.rt = round(as.numeric(Resp.rt)),
...
# Saving
save(coords,dataset_full,dataset_gw1,dataset_neutral,file = file.path("objects", "online_circular.RData"))
...
df <- accuracy %>%
  ...
  ungroup()%>%
  mutate(label = paste0(subject,video_set,emotion))
...
df <- left_join(df,intensity_mean, by = "label") %>%
  select(-label) %>%
  'colnames<-'(c("subject" ,"video_set", "emotion",  "accuracy","intensity"))


#### built dataset
write.csv(df, "objects/behavioral_data_study1.csv", row.names = FALSE)
