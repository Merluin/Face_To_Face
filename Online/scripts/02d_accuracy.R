...
dat_summ <- dataset_full %>%
  ...
  dplyr::select(Pt.code,  Video.set, Video.emotion, Wheel.name, Resp.category, Resp.correct) %>%
  mutate(Resp.correct = 1)%>%
  group_by(Pt.code,Video.emotion,Video.set,Wheel.name) %>%
  mutate(n = n())%>%
  group_by(Pt.code,Video.emotion,Video.set,Resp.category,Wheel.name) %>%
  mutate( Resp.correct = sum(Resp.correct),
          Resp.distribution = Resp.correct/n)%>%
  summarize(Resp.distribution = mean(Resp.distribution))%>%
  group_by(Video.emotion,Video.set,Resp.category,Wheel.name) %>%
  summarize(Resp.distribution = mean(Resp.distribution))
...
plot_gew_discrete <- dat_summ %>% 
  ...
  ggplot(aes(x = Resp.category, y = Resp.distribution, fill = Video.set)) +
...
# Fit  correct for each emotion--------------------------------------------
  ...
  fit <- glm(correct ~ emotion * video_set , data = dataset_glm, family = binomial)
  ...
  cowplot::plot_grid(video,emotion)
