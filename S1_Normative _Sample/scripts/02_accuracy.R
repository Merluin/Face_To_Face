...
write.csv(df, "objects/dataset_anova_accuracy.csv", row.names = FALSE)

df <- df %>%
  group_by(subject,video_set) %>%
  summarise( acc = mean(acc, na.rm = TRUE))
write.csv(df, "objects/online_accuracy_summary.csv", row.names = FALSE)
...
df_wide <- df %>% 
  pivot_wider(names_from = video_set, values_from = acc) %>%
  drop_na()
...
# Summarize accuracy per subject per video_set (average across emotions)
df_agg <- df 
...
percentili_90 <- df %>%
  ...
  group_by(video_set) %>%
  summarise(p05 = quantile(mean_acc, 0.05),
            p95 = quantile(mean_acc, 0.95))
print(percentili_90)
...
# Create plot
p1 <- ggplot(df[df$video_set %in% c("ADFES","JeFEE"),], aes(video_set, acc, fill = video_set)) +
  geom_rain(alpha = .5, rain.side = 'f1x1', id.long.var = "subject") +
  geom_point(data = descritive, aes(x = c(1.1, 1.9), y = mean_acc), color = "black", size = 2.5, shape = 18) +
  geom_errorbar(data = descritive,
                mapping = aes(x = c(1.1, 1.9), ymin = mean_acc - se, ymax = mean_acc + se),
                inherit.aes = FALSE,
                width = 0, color = "black", linewidth = 0.5) +
  scale_fill_manual(values = colors) +
  labs(x = NULL, y = "Mean Accuracy", fill = NULL) +
